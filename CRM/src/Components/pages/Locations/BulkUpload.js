...
import { bulkUploadLocations } from "../../../Utils/Requests/Location";
...
function BulkUpload({ switchMode }) {
  ...
  const { locations, loading, fileName } = state;

  const mapRow = row => {
    const normalized = {};
    for (let key in row) {
      const normKey = key.trim().toLowerCase().replace(/[\s_-]+/g, "");
      if (normKey === "name" || normKey === "locationname" || normKey === "location") {
        normalized.name = row[key];
      } else if (normKey === "district" || normKey === "districts") {
        normalized.district = row[key];
      }
    }
    return normalized;
  };
  ...
  const handleFileChange = e => {
    ...
    reader.onload = evt => {
      try {
        ...
        const parsedLocations = rawRows.map(row => mapRow(row));
        ...
        Swal.fire({
          type: "success",
          title: `Parsed ${parsedLocations.length} locations successfully!`,
          text: "Review the locations in the preview table below and click upload."
        });
      } catch (err) {
  ...
  const handleUpload = async () => {
    ...
    const resp = await bulkUploadLocations({ locations }).catch(err => {
    ...
    if (resp && resp.status === 200) {
      ...
      if (errorCount > 0) {
        ...
        Swal.fire({
          ...
          text: `Successfully uploaded ${successCount} locations. ${errorCount} failed.`,
        ...
        Swal.fire({
          ...
          text: `Successfully uploaded all ${successCount} locations!`,
          onClose: () => {
            window.location.href = "/locations";
          }
        });
  ...
  return (
    <Layout title="Bulk Location Upload (Excel / CSV)">
      ...
      <div className="card card-default" style={{ padding: "1.5rem", marginBottom: "1.5rem" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          <h4>Instructions for Bulk Location Upload</h4>
          <p style={{ color: "#666", fontSize: "0.95rem" }}>
            Upload an Excel (`.xlsx`) or CSV (`.csv`) sheet containing your locations list. Make sure the sheet columns match the following headers (case and spaces are flexible):
          </p>
          <div style={{ background: "#f8f9fa", color: "#080d18", padding: "10px", borderRadius: "4px", fontSize: "0.88rem", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: "8px" }}>
            <div><b>Name</b> (e.g. Kathmandu)</div>
            <div><b>District</b> (e.g. Achham)</div>
          </div>
        </div>
        ...
        <div style={{ marginTop: "1rem" }}>
          ...
          {fileName && (
            <button className="btn btn-warning" onClick={handleUpload} style={{ marginLeft: "1rem", padding: "0.8rem 1.5rem" }} disabled={loading}>
              {loading ? "Uploading..." : `Upload ${locations.length} Locations`}
            </button>
          )}
        </div>
      </div>
      ...
      {locations.length > 0 && (
        <div className="card card-default" style={{ padding: "1.5rem" }}>
          <h4>Previewing Locations ({locations.length} rows)</h4>
